...
def books_list(request):
    ...
    context = {'books': books}
    return render(request, 'academic/books_list.html', context)
...
def add_book(request):
    if request.method == 'POST':
        ...
        form = BookForm()
        context = {'form': form}
    return render(request, 'academic/add_book.html', context)
...
def book_detail(request, pk):
    try:
        ...
        context = {'book': book}
        return render(request, 'academic/book_detail.html', context)
    except ObjectDoesNotExist:
        context = {'message': 'الكتاب غير موجود.'}
        return render(request, 'academic/book_not_found.html', context)
...
def college_links(request):
    ...
    context = {'links': links}
    return render(request, 'academic/college_links.html', context)
...
def search_books(request):
    ...
    return render(request, 'academic/search_books.html', context)
    
# def search_books(request):
#     query = escape(request.GET.get('q', '').strip())
#     books = academic.objects.filter(
#         Q(title__icontains=query) | Q(author__icontains=query)
#         ) if query else academic.objects.all() # list all books
